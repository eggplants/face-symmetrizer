...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "images", metavar="file", type=str, nargs="+", help="input image files"
    )
    ...
    parser.add_argument("-q", "--quiet", action="store_true", help="make log quiet")
    parser.add_argument(
        "-o",
        "--outdir",
        default=".",
...
def _save(f: FaceSym, args: argparse.Namespace) -> None:
    ...
    for face_id in range(f.face_count):
        print("[+]face:", face_id)
        ims = f.get_symmetrized_images(idx=face_id, show=args.show)
